...
if __name__ == "__main__":
	...
	no_result_counter = 0
	X_all = {variation : [] for variation in variations}
	Y_all = {variation : [] for variation in variations}
	for row in data:
		try:
			Y_all[row["variation"]].append(float(row["result"]))
		except:
			no_result_counter += 1
		else:
			try:
				X_all[row["variation"]].append([float(observed) for observed in row["userData"]])
			except:
				print(e)
				sys.exit(1)
	...
	for var_id, var_data in Y_all.iteritems():
		if len(var_data) < 200:
			raise RuntimeError(str(len(var_data))"/200: Not enough samples yet for variation " + var_id)
	...
	model_props = {
		"l1_ratio" : [.1, .25, .5, .7, .8, .85, .9, .925, .95, .97, .99, 1], #0.5 default
		"eps" : 0.001,
		"n_alphas" : 100,
		"alphas" : None,
		"fit_intercept" : True,
	...
	# setup trained funcs
	fitted_pars = {variation : None for variation in variations}
	...
	for var_id in X_all:
		...
		fitted_pars[var_id] = fitted_model.decision_function(X_all[var_id])
	

	result_updaet = db.modules.update_one(
		{
		'_id' : object_id
		},
		{
